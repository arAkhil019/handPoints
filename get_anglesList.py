...
def get3dAngle(p1, p2, p3):
    ...
    return round((180-angle),2)

def get_anglesList(landmarks):
    lm = landmarks
    angles = {}
    for i in range(1,21,4):
        a1 = get3dAngle(lm[i],lm[i+1],lm[i+2])
        a2 = get3dAngle(lm[i+1],lm[i+2],lm[i+3])
        angles[i+1] = a1
        angles[i+2] = a2
    return angles
